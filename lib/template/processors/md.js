...
function argParser (str) {
  ...
  throw new TerraformError({
    source: "Markdown shortcode",
    dest: "HTML",
    message: "unrecognized Argument: " + str
  });
}
...
function shortcodeRule (state, startLine, endLine, silent) {
  ...
  case 'partial':
    ...
    if (typeof path !== 'string') throw new TerraformError({
      source: "Markdown shortcode",
      dest: "HTML",
      message: 'Argument type mismatch, expected "string", got ' + typeof path
    });
    if (locals && typeof locals !== 'object') throw new TerraformError({
      source: "Markdown shortcode",
      dest: "HTML",
      message: 'Argument type mismatch, expected "object", got ' + locals.toString()
    });
